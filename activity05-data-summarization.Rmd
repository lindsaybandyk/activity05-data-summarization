...
**Response**:
i keep getting an error with out the parenthessis because it does not focus on the correct platform, it stops the messages tobe able to be seen. but with the parenthesis ut makes the code execute and print at the same time!
...
- Inspired: Ctrl/Command + Alt/Option + 
...
```{r summary of earnings code chunk}
...
i think that based on the distribution of median earnings that it should be bnetween $36,000 and $40,151.45 and it is skewed right. 
...
```{r major earnings data summarization}
...
**Response**:
 as for the income median it was almost all at or below $40,000 and it fluctuates but is still skewed to the right. 
...
it makes the code look better and more presentable.
...
for all of the majors the highest is around $46000 and the lowesr around $25,000. 
### Multiple Rankings
...
```{r ranking by category and aplhabetically}
college_recent_grads %>% 
  group_by(major_category) %>% 
...
**Response**:
most of the female majors include psychology, Health, and interdisciplinary and the least include engineering and Computer/Math
